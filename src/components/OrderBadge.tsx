...
export const OrderBadge: React.FC<OrderBadgeProps> = ({ orderKey, showDetails = false }) => {
  ...
  if (!detail) {
    return (
      <span className="px-2 py-0.5 bg-white/5 border border-white/10 text-gray-700 text-[10px] font-bold font-mono">
        {orderKey}
      </span>
    );
  }
  ...
  return (
    <div className="border p-4 glass shadow-2xs space-y-2 border-l-4 text-left rtl:text-right" style={{ borderLeftColor: detail.badgeColor }}>
      ...
      <div className="pt-2 text-[10px] text-gray-500 font-mono space-y-1 border-t border-gray-100">
